 ...
 */
//static
 int recursive_truncate_blocks(struct super_block *sb, __le64 block,
...
//static 
void pmfs_decrease_btree_height(struct super_block *sb,
 ...
 * Free data blocks from inode in the range start <=> end
 */
void pmfs_truncate_blocks(struct inode *inode, loff_t start, loff_t end)
{
	__pmfs_truncate_blocks(inode, start, end);
}
int pmfs_increase_btree_height(struct super_block *sb,
 ...
 */
//static
int recursive_alloc_blocks(pmfs_transaction_t *trans,
...
void kernelfs_update_inode(struct inode *inode, struct pmfs_inode *pi)
{
...
int pmfs_increase_inode_table_size(struct super_block *sb)
{
	struct pmfs_sb_info *sbi = PMFS_SB(sb);
	struct pmfs_inode *pi = pmfs_get_inode_table(sb);
	pmfs_transaction_t *trans;
