...
extern void kernelfs_update_inode(struct inode *inode, struct pmfs_inode *pi);
extern void pmfs_put_inode(struct inode *inode);
extern void pmfs_evict_inode(struct inode *inode);
extern int pmfs_increase_inode_table_size(struct super_block *sb);
...
int pmfs_getattr(struct vfsmount *mnt, struct dentry *, struct kstat *);
...
extern int pmfs_increase_btree_height(struct super_block *sb,
		struct pmfs_inode *pi, u32 new_height);
extern void pmfs_truncate_blocks(struct inode *inode, loff_t start, loff_t end);
extern int recursive_truncate_blocks(struct super_block *sb, __le64 block,
	u32 height, u32 btype, unsigned long first_blocknr,
	unsigned long last_blocknr, bool *meta_empty);
extern void pmfs_decrease_btree_height(struct super_block *sb,
	struct pmfs_inode *pi, unsigned long newsize, __le64 newroot);
extern int recursive_alloc_blocks(pmfs_transaction_t *trans,
...
static inline struct pmfs_inode *pmfs_fetch_inode(struct super_block *sb,
						  u64	ino)
{
	struct pmfs_super_block *ps = pmfs_get_super(sb);
	struct pmfs_inode *inode_table = pmfs_get_inode_table(sb);
...
//extern struct inode_lru_t *inodelru;
//extern unsigned long logaddr;
//extern void scaninode(struct super_block *sb);
//extern int migrate_blocks(struct super_block *sb);
//======================define syscall===============================
extern void init_operatesb_call(void);
extern void exit_operatesb_call(void);
