		    ...
		    loff_t *ppos)
{
	...
	do {
		...
		//unsigned long long readtime = (end.tv_sec - start.tv_sec) * 1000000000 + (end.tv_nsec - start.tv_nsec);
		//printk("nr %d readtime %d\n",nr, readtime);
		if (left) {
			error = -EFAULT;
			goto out;
		}
          ...
          size_t count, loff_t pos, loff_t *ppos)
{
	...
	do {
		...
		//getrawmonotonic(&end);
		//unsigned long long writetime = (end.tv_sec - start.tv_sec) * 1000000000 + (end.tv_nsec - start.tv_nsec);
		//printk("writetime %dns\n",writetime);
